...
class PhraseNodeTrainingRun(TorchTrainingRun):
    ...
    def test(self): 
        print("STARTING TEST")
        filename = join(self.workspace.logs, 'eval-pn-test.{}.gz'.format(99999))
        print(type(filename))
        print(filename)
        with gzip.open(filename, 'w') as test_logfile:
            ...
            print 'TEST @ {}: {}'.format(99999, test_stats)
            test_stats.log(self.tb_logger, 99999, 'pn_test_', ignore_grad_norm=True)
